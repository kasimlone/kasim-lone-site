...
export function ProjectTile({ project, index = 0 }: { project: Project; index?: number }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group block rounded-lg overflow-hidden border border-zinc-700/60 bg-panel/70 backdrop-blur-md transition-all hover:-translate-y-1 hover:border-syntax-keyword hover:bg-panel/85 hover:shadow-[0_12px_32px_rgba(0,0,0,0.5)]"
    >
      {/* Tab bar */}
      <div className="flex items-center gap-2 bg-bg2/80 border-b border-zinc-700/60 px-3 py-1.5">
        ...
        <ArrowUpRight size={12} className="ml-auto text-zinc-500 group-hover:text-syntax-keyword transition" />
      </div>
